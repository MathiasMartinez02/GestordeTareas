...
def render_todos():
    ...
    for i in range(0, len(rows)):
        ...
        color = '#C0C0C0' if completed else '#000000'
        cBtn = Checkbutton(frame, text=description, fg=color, width=42,
                           anchor='w', command=complete(id))
        cBtn.grid(row=i, column=0, sticky='w')
        xBtn = Button(frame, text='Eliminar', command=remove(id))
        xBtn.grid(row=i, column=1)
